...
export function InspectorControls( props ) {
	...
	const setHasOverlay = useCallback( ( value ) => {
		setAttributes( {
			hasOverlay: value,
		} );
	} );

	const toggleOverlayHasParallax = useCallback( () => {
		setAttributes( {
	...
	const toggleOverlayIsRepeated = useCallback( () => {
		setAttributes( {
			overlayIsRepeated: ! overlayIsRepeated,
		} );
	} );

	const setOverlayColor = useCallback( ( value ) => {
		setAttributes( {
			overlayColor: value,
		} );
	} );

	const setOverlayColorOpacity = useCallback( ( value ) => {
		setAttributes( {
			overlayColorOpacity: value,
		} );
	} );

	const setOverlayFocalPoint = useCallback( ( value ) => {
		setAttributes( {
			overlayFocalPoint: value,
		} );
	} );

	const setOverlayAltText = useCallback( ( value ) => {
		setAttributes( {
	...
	const setFollowUp = useCallback( ( value ) => {
		setAttributes( {
			followUp: value,
		} );
	} );

	const imperativeFocalPointPreview = useCallback( ( value ) => {
